...
window.pannellum = (function() {
    ...
    lib.viewer = function(container, config) {
        ...
        if (!gl) {
            el.innerHTML = "<p style='color:white;padding:20px;'>WebGL Not Supported</p>";
            return;
        }
        ...
        for (var latNum = 0; latNum <= latBands; latNum++) {
            ...
            for (var lonNum = 0; lonNum <= lonBands; lonNum++) {
                ...
                vertices.push(radius * sinTheta * sinPhi, radius * cosTheta, radius * sinTheta * cosPhi);
            }
        }
        for (var latNum = 0; latNum < latBands; latNum++) {
            for (var lonNum = 0; lonNum < lonBands; lonNum++) {
        ...
        var posAttr = gl.getAttribLocation(prog, "a_pos");
        ...
        function drawScene() {
            ...
            mat4Multiply(modelView, rotP, rotY);
        ...
        if (config.hotSpots) {
            config.hotSpots.forEach(function(sp) {
                ...
                if (sp.createTooltipFunc) sp.createTooltipFunc(btn, sp.createTooltipArgs);
